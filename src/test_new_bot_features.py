...
class TestNewBotFeatures:
    ...
    async def test_show_stats(self, mock_update, mock_context):
        ...
        with patch('bot.stats_manager') as mock_stats_manager:
            mock_stats_manager.get_user_summary.return_value = "📊 **Bot Usage Statistics**\n👥 Total Users: 5"
    ...
    async def test_test_search_now_success(self, mock_update, mock_context):
        ...
        with patch('bot.IdealistaScraper') as mock_scraper_class:
            ...
            mock_scraper.scrape_listings = AsyncMock(return_value=[
                {"title": "Test Listing", "link": "https://test.com"}
            ])
            ...
            final_call_args = mock_update.callback_query.message.edit_text.call_args[0][0]
...
class TestEnhancedMonitoringFlow:
    ...
    @pytest.mark.asyncio
    async def test_start_monitoring_with_task_validation(self, mock_update, mock_context):
    ...
    @pytest.mark.asyncio
    async def test_start_monitoring_task_fails_immediately(self, mock_update, mock_context):
        ...
        with patch('bot.user_monitoring_task') as mock_task_func:
            ...
            with patch('asyncio.create_task', return_value=mock_task):
                ...
                # Task should be removed from monitoring_tasks
            ...
            with patch('asyncio.sleep', return_value=None):  # Skip sleep
                with patch('bot.user_configs', {12345: SearchConfig()}):
                    try:
                        ...
                        pytest.fail(f"Monitoring task should handle scraping errors: {e}")
...
class TestMainMenuEnhancements:
    ...
    def test_main_menu_includes_new_options(self):
        ...
        assert "Reset settings" in menu_text
    ...
    def test_main_menu_dynamic_monitoring_button(self):
        ...
        keyboard = bot.get_main_menu_keyboard(12345)
        menu_text = str(keyboard)
        assert "Start searching" in menu_text
        ...
        keyboard = bot.get_main_menu_keyboard(12345)
        menu_text = str(keyboard)
        assert "Stop monitoring" in menu_text
    ...
    async def test_button_handler_routes_new_commands(self, mock_update, mock_context):
        ...
        mock_update.callback_query.data = 'check_status'
        with patch('bot.check_monitoring_status', return_value=bot.CHOOSING) as mock_check_status:
            result = await bot.button_handler(mock_update, mock_context)
        ...
        mock_update.callback_query.data = 'test_search'
        with patch('bot.test_search_now', return_value=bot.CHOOSING) as mock_test_search:
            result = await bot.button_handler(mock_update, mock_context)
    ...
    async def test_monitoring_task_logs_start(self):
        ...
        with patch('bot.logger') as mock_logger:
            with patch('bot.IdealistaScraper') as mock_scraper_class:
                ...
                mock_logger.info.assert_any_call("MONITORING STARTED: User 12345 monitoring task is now running")
    ...
    def test_config_loading_logs_details(self):
        ...
        with patch('builtins.open'), patch('json.load', return_value=mock_config_data):
            with patch('bot.logger') as mock_logger:
                ...
                mock_logger.info.assert_any_call("Loaded config for user 12345: {'min_rooms': 2, 'max_rooms': 10, 'min_size': 30, 'max_size': 200, 'max_price': 1500, 'furniture_types': [<FurnitureType.FURNISHED: 'mobilado'>], 'property_states': [<PropertyState.GOOD: 'bom-estado'>], 'city': 'lisboa', 'custom_polygon': None, 'update_frequency': 5}")
        ...
        with patch('bot.logger') as mock_logger:
            with patch('bot.user_monitoring_task') as mock_task_func:
                ...
                with patch('asyncio.create_task', return_value=mock_task):
                    ...
                    mock_logger.info.assert_any_call(f"DEBUG: Monitoring task created for user 12345, task ID: {id(mock_task)}")
                    mock_logger.info.assert_any_call("SUCCESS: Monitoring task for user 12345 is running properly")