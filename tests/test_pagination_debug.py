...
async def test_pagination_debug():
    ...
    with patch('scraper.fetch_page', side_effect=mock_fetch_page):
        results = await scraper.scrape_listings(config, "test_user", max_pages=3, force_all_pages=False)
    ...
    scraper.seen_listings = {"test_user": {
    ...
    with patch('scraper.fetch_page', side_effect=mock_fetch_page):
        results = await scraper.scrape_listings(config_polygon, "test_user", max_pages=2, force_all_pages=True)
