...
class TestCompleteMultiUserFlow:
    ...
    async def test_complete_user_journey(self, mock_update, mock_context, temp_files):
        ...
        with patch("bot.IdealistaScraper") as mock_scraper_class:
            ...
            mock_scraper.scrape_listings = AsyncMock(
                return_value=[{"title": "Test Apartment", "link": "https://test.com/1"}]
            )
    ...
    async def test_configuration_persistence_with_debugging(self, temp_files):
        ...
        with (
            patch("builtins.open"),
            patch("json.load", return_value=mock_config_with_invalid_fields),
        ):
            bot.user_configs.clear()
            bot.load_configs()
...
class TestErrorScenarios:
    ...
    async def test_scraper_rate_limit_adaptive_behavior(self):
        ...
        expected_delay = min(
            initial_delay * (limiter.backoff_multiplier**3), limiter.max_delay
        )
        ...
        if limiter.recent_errors > 0 and (current_time - limiter.last_error_time) < 300:
            calculated_delay = min(
                limiter.min_delay_seconds
                * (limiter.backoff_multiplier**limiter.recent_errors),
                limiter.max_delay,
            )
    ...
    async def test_configuration_corruption_handling(self):
        ...
        corruption_scenarios = [
            ...
            # Invalid enum values
            '{"12345": {"furniture_type": "invalid_enum", "property_states": ["also_invalid"]}}',
        ]

        for corrupted_data in corruption_scenarios:
            with patch("builtins.open", create=True):
                try:
                    # Try to parse JSON - if it's valid JSON, test with valid structure
                    ...
                    with patch("json.load", return_value=parsed_data):
                        try:
                            ...
                            assert not isinstance(
                                e, (json.JSONDecodeError, KeyError)
...
class TestBackwardsCompatibility:
    ...
    async def test_seen_listings_compatibility(self):
        ...
        with (
            patch("builtins.open"),
            patch("json.load", return_value=existing_seen_listings),
        ):
