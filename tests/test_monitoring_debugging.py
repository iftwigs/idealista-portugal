...
class TestMonitoringTaskManagement:
    ...
    @pytest.mark.asyncio
    async def test_monitoring_task_creation_and_validation(
        ...
        with patch("bot.IdealistaScraper") as mock_scraper_class:
            ...
            with patch("bot.logger") as mock_logger:
                ...
                mock_logger.warning.assert_any_call(
                    "Rate limit encountered for user 12345 - will retry next cycle with longer delays"
                )
...
class TestMonitoringStatusChecking:
    ...
    @pytest.mark.asyncio
    async def test_check_monitoring_status_inactive_task(
        self, mock_update, mock_context
    ):
    ...
    @pytest.mark.asyncio
    async def test_check_monitoring_status_rate_limiting_info(
        self, mock_update, mock_context
    ):
        """Test that status check includes rate limiting information"""
        bot.user_configs[12345] = SearchConfig()

        with patch("scraper.global_rate_limiter") as mock_limiter:
...
class TestDebuggingUtilities:
    ...
    def test_monitoring_task_logging_format(self):
        ...
        assert (
            "MONITORING STARTED:"
            in "MONITORING STARTED: User 12345 monitoring task is now running"
        )
...
class TestIntegrationWithExistingFeatures:
    ...
    async def test_status_check_button_in_test_results(self, mock_update, mock_context):
        ...
        with patch("bot.IdealistaScraper") as mock_scraper_class:
            ...
            reply_markup = mock_update.callback_query.message.edit_text.call_args[1][
                "reply_markup"
            ]
    ...
    @pytest.mark.asyncio
    async def test_status_check_includes_test_search_button(
        self, mock_update, mock_context
    ):
    ...
    async def test_button_routing_for_new_commands(self, mock_update, mock_context):
        ...
        for callback_data, expected_function in test_commands:
            ...
            with patch(
                f"bot.{expected_function}", return_value=bot.CHOOSING
            ) as mock_func:
