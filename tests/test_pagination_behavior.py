...
class TestPaginationBehavior:
    ...
    @pytest.mark.asyncio
    async def test_pagination_scrapes_all_pages_with_force_all_pages(
        self, scraper, config
    ):
        ...
        with patch("scraper.fetch_page", side_effect=mock_fetch_page):
            results = await scraper.scrape_listings(
                config, "test_user", max_pages=5, force_all_pages=True
            )
    ...
    @pytest.mark.asyncio
    async def test_pagination_stops_early_without_force_all_pages(
        self, scraper, config
    ):
        """Test that pagination stops early when no new listings found and force_all_pages=False"""

        # Add some "seen" listings to simulate previously scraped content
        # Add all listings from page 2 to seen_listings
        scraper.seen_listings["test_user"].add(
        ...
        async def mock_fetch_page(session, url, user_id=None):
            print(f"MOCK FETCH PAGE called for URL: {url}")
            if "pagina=2" in url:
                # Page 2 has listings but they're all "seen" (already in seen_listings)
                html = self.create_mock_html_with_listings(2, page_num=2)
                print(f"Returning page 2 HTML with 2 listings (all seen)")
                return html
            elif "pagina=3" in url:
                # Page 3 has no listings, should trigger empty page logic
                print(f"Returning page 3 HTML with 0 listings (empty page)")
                return "<html><body>No listings found</body></html>"
            elif "pagina=4" in url:
                # This should not be reached due to early stopping
                print("ERROR: Page 4 should not be reached!")
                pytest.fail("Page 4 should not be reached due to early stopping")
            else:  # page 1
                html = self.create_mock_html_with_listings(5, page_num=1)
                print(f"Returning page 1 HTML with 5 listings")
                return html

        with patch("scraper.fetch_page", side_effect=mock_fetch_page):
            results = await scraper.scrape_listings(
                config, "test_user", max_pages=5, force_all_pages=False
            )

        # Should only get listings from page 1 (5 listings)
        # Page 2 listings are "seen" (0 new) and page 3 is empty, triggering early stop after 2 consecutive empty pages
    ...
    @pytest.mark.asyncio
    async def test_pagination_continues_past_empty_pages_early_in_sequence(
        self, scraper, config
    ):
        ...
        scraper.seen_listings["test_user"].add(
            "https://www.idealista.pt/property/listing_1_0"
        )
        scraper.seen_listings["test_user"].add(
            "https://www.idealista.pt/property/listing_2_0"
        )
        ...
        async def mock_fetch_page(session, url, user_id=None):
            if "pagina=2" in url:
                return self.create_mock_html_with_listings(
                    1, page_num=2
                )  # Seen listing
            elif "pagina=3" in url:
                return self.create_mock_html_with_listings(
        ...
        with patch("scraper.fetch_page", side_effect=mock_fetch_page):
            results = await scraper.scrape_listings(
                config, "test_user", max_pages=4, force_all_pages=False
            )
    ...
    async def test_pagination_url_construction_city_based(self, scraper, config):
        ...
        assert (
            "pagina=" not in expected_urls[0]
        )  # Page 1 should not have pagina parameter
    ...
    async def test_pagination_respects_max_pages_limit(self, scraper, config):
        ...
        with patch("scraper.fetch_page", side_effect=mock_fetch_page):
            results = await scraper.scrape_listings(
                config, "test_user", max_pages=2, force_all_pages=True
            )
    ...
    @pytest.mark.asyncio
    async def test_pagination_seen_listings_tracking_across_pages(
        self, scraper, config
    ):
        ...
        with patch("scraper.fetch_page", side_effect=mock_fetch_page):
            # First scrape
            results1 = await scraper.scrape_listings(
                config, "test_user", max_pages=2, force_all_pages=True
            )

            # Second scrape (should find all listings as "seen")
            results2 = await scraper.scrape_listings(
                config, "test_user", max_pages=2, force_all_pages=True
            )
...
if __name__ == "__main__":
    ...
    async def run_all_tests():
        ...
        scraper.seen_listings = {"test_user": set()}
        await run_test(
            test_instance.test_pagination_scrapes_all_pages_with_force_all_pages,
            scraper,
            config,
        )
        ...
        scraper.seen_listings = {"test_user": set()}
        await run_test(
            test_instance.test_pagination_stops_early_without_force_all_pages,
            scraper,
            config,
        )
        ...
        scraper.seen_listings = {"test_user": set()}
        await run_test(
            test_instance.test_pagination_continues_past_empty_pages_early_in_sequence,
            scraper,
            config,
        )
        ...
        scraper.seen_listings = {"test_user": set()}
        await run_test(
            test_instance.test_pagination_stops_on_consecutive_empty_pages,
            scraper,
            config,
        )
        ...
        scraper.seen_listings = {"test_user": set()}
        await run_test(
            test_instance.test_pagination_url_construction_city_based, scraper, config
        )

        scraper.seen_listings = {"test_user": set()}
        await run_test(
            test_instance.test_pagination_url_construction_custom_polygon, scraper
        )

        scraper.seen_listings = {"test_user": set()}
        await run_test(
            test_instance.test_pagination_respects_max_pages_limit, scraper, config
        )

        scraper.seen_listings = {"test_user": set()}
        await run_test(
            test_instance.test_pagination_handles_fetch_errors_gracefully,
            scraper,
            config,
        )
        ...
        scraper.seen_listings = {"test_user": set()}
        await run_test(
            test_instance.test_pagination_seen_listings_tracking_across_pages,
            scraper,
            config,
        )
