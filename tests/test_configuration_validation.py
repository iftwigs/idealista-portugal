...
class TestConfigurationValidation:
    ...
    def test_valid_configuration_fields(self):
        ...
        test_config = {
            ...
            "max_size": 100,
            "max_price": 1500,
            "furniture_type": FurnitureType.FURNISHED,
    ...
    def test_search_config_with_unknown_kwargs(self):
        ...
        invalid_config = {**valid_config, "invalid_field": "value"}
    ...
    def test_backwards_compatibility_has_furniture(self):
        ...
        with patch("builtins.open"), patch("json.load", return_value=mock_config_data):
            ...
            # Should convert has_furniture to furniture_type
    ...
    def test_backwards_compatibility_furniture_type(self):
        ...
        with patch("builtins.open"), patch("json.load", return_value=mock_config_data):
            ...
            # Should convert furniture_type to furniture_type
            assert config.furniture_type == FurnitureType.FURNISHED
    ...
    def test_multiple_backwards_compatibility_conversions(self):
        ...
        mock_config_data = {
            "12345": {
                ...
                "max_rooms": 4,
                "max_price": 1500,
                "has_furniture": False,  # Old format -> INDIFFERENT
    ...
    def test_config_loading_with_invalid_enum_values(self):
        ...
        mock_config_data = {
            "12345": {
                ...
                "max_rooms": 4,
                "max_price": 1500,
                "furniture_type": "invalid_furniture_type",  # Invalid enum
    ...
    def test_config_field_filtering_preserves_order(self):
        ...
        filtered_config = {
            k: v for k, v in original_config.items() if k in valid_fields
        }
        ...
        assert filtered_config["furniture_type"] == FurnitureType.FURNISHED
...
class TestConfigurationPersistence:
    ...
    def test_config_serialization_format(self):
        ...
        assert user_config["furniture_type"] == "equipamento_mobilado"
        assert user_config["property_states"] == ["bom-estado", "com-novo"]
        assert isinstance(user_config["furniture_type"], str)
    ...
    def test_load_configs_file_not_found(self):
        ...
        with patch("builtins.open", side_effect=FileNotFoundError()):
            with patch("bot.logger") as mock_logger:
                ...
                # Should handle gracefully and log message
                mock_logger.info.assert_called_with(
                    "user_configs.json not found, will create on first save"
                )
        ...
        with (
            ...
            with patch("bot.logger") as mock_logger:
                ...
                mock_logger.warning.assert_called_with(
                    "Invalid JSON in user_configs.json, will recreate on next save"
                )
    ...
    def test_load_configs_corrupted_enum_values(self):
        ...
        corrupted_config = {
            "12345": {
                "min_rooms": 2,
                "max_price": 1500,
                "furniture_type": "INVALID_FURNITURE_TYPE",
        ...
        with patch("builtins.open"), patch("json.load", return_value=corrupted_config):
            ...
            try:
                ...
                if 12345 in bot.user_configs:
                    config = bot.user_configs[12345]
                    assert isinstance(config.furniture_type, FurnitureType)
                    assert isinstance(config.property_states, list)
                    assert all(
                        isinstance(ps, PropertyState) for ps in config.property_states
                    )
